...
class CheckMusic(object):
    def __init__(self):
        ...
        # 读取音乐表信息
        self.df = self.read_musicinfo_excel()
        self.song_list_from_music_xls = list(set(self.df['id_mode_level'].tolist()))
    ...
    def process_assetbundle(self):
        ...
        dance_txt_list_from_music_xls = list(set(self.df['dancetxt_file_dir'].tolist()))
    ...
    def process_stage(self):
        ...
        adb_stage_list_from_music_xls = list(set(self.df['adb_stagefile_dir'].tolist()))
        ios_stage_list_from_music_xls = list(set(self.df['ios_stagefile_dir'].tolist()))
        # 获取两个 list 的差集
    ...
    def process_music_smp(self):
        ...
        music_file_list_from_music_xls = list(set(self.df['music_file_dir'].tolist()))
    ...
    def read_musicinfo_excel(self):
        musicinfo_xls = os.getcwd() + '\\xls\\音乐表.xlsx'
        if not os.path.exists(musicinfo_xls):
            self.mylogger.error("音乐表.xlsx文件缺失!")
            exit()

        music = pd.read_excel(musicinfo_xls, sheetname='关卡表')
        music['songid'] = music['歌曲ID'].apply(lambda x: str(x).zfill(4))
        music['modestr'] = music['模式ID'].apply(lambda x: self.modestr_dict.get(x))
        music['mode_filename'] = music['模式ID'].apply(lambda x: self.mode_filename_dict.get(x))
        music['levelstr'] = music['难度'].apply(lambda x: self.levelstr_dict.get(x))
        music['adb_stagefile_dir'] = self.stage_dir + "\\android\\" + music['mode_filename'] + "\\song" + music[
            'songid'] + "." + music['modestr'] + music['levelstr']
        music['ios_stagefile_dir'] = self.stage_dir + "\\ios\\" + music['mode_filename'] + "\\song" + music[
            'songid'] + "." + music['modestr'] + music['levelstr']
        music['music_file_dir'] = self.music_file_dir + "\\song" + music['songid'] + ".smp"
        music['dancetxt_file_dir'] = self.controller_txt_dir + "\\song" + music['songid'] + "\\Dance.txt"
        music['id_mode_level'] = music['歌曲ID'].map(str) + ',' + music['模式ID'].map(str) + ',' + music['难度'].map(str)
        music_new = music[['歌曲名', 'id_mode_level', 'music_file_dir',
                           'adb_stagefile_dir', 'ios_stagefile_dir', 'dancetxt_file_dir']]
        music_new.rename(columns={'歌曲名': 'name'}, inplace=True)
        music_new.reset_index(drop=True, inplace=True)
        music_new.to_excel("result.xlsx", index=False)  # write data to excel
        return music_new

    def process_dama(self):
        xls = os.getcwd() + "\\xls\\广场舞大妈.xlsx"
        if not os.path.exists(xls):
            self.mylogger.error("广场舞大妈.xlsx文件缺失!")
            exit()
    ...
    def process_lwstar(self):
        xls = os.getcwd() + "\\xls\\恋舞之星.xlsx"
        if not os.path.exists(xls):
            self.mylogger.error("恋舞之星.xlsx文件缺失!")
            exit()
    ...
    def process_magiclamp(self):
        xls = os.getcwd() + "\\xls\\魔法神灯.xlsx"
        if not os.path.exists(xls):
            self.mylogger.error("魔法神灯.xlsx文件缺失!")
            exit()
    ...
    def process_fairlyland(self):
        xls = os.getcwd() + "\\xls\\舞团秘境.xlsx"
        if not os.path.exists(xls):
            self.mylogger.error("舞团秘境.xlsx文件缺失!")
            exit()
    ...
    def process_starmentor(self):
        xls = os.getcwd() + "\\xls\\星恋挑战.xlsx"
        if not os.path.exists(xls):
            self.mylogger.error("星恋挑战.xlsx文件缺失!")
            exit()
    ...
    def process_diamondleague(self):
        xls = os.getcwd() + "\\xls\\钻石联赛.xlsx"
        if not os.path.exists(xls):
            self.mylogger.error("钻石联赛.xlsx文件缺失!")
            exit()
    ...
    def process_musicrank(self):
        xls = os.getcwd() + "\\xls\\音悦榜.xlsx"
        if not os.path.exists(xls):
            self.mylogger.error("音悦榜.xlsx文件缺失!")
            exit()
